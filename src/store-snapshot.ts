...
export type SnapshotExportBindings = {
  ...
  readScopedResumeRowsSync(sessionIDs?: string[]): Array<{ session_id: string; note: string; updated_at: number }>;
...
export async function exportStoreSnapshot(bindings: SnapshotExportBindings, input: ExportSnapshotInput): Promise<string> {
...
export async function importStoreSnapshot(bindings: SnapshotImportBindings, input: ImportSnapshotInput): Promise<string> {
...
function listSnapshotWorktreeKeys(rows: Array<{ session_directory: string | null; worktree_key: string | null }>): string[] {
  return [...new Set(rows.map((row) => row.worktree_key ?? normalizeWorktreeKey(row.session_directory ?? undefined)).filter(Boolean))] as string[];
}
...
function rehomeImportedSessionRow(session: SessionRow, directory: string, worktreeKey?: string): SessionRow {
  return {
    ...session,
