...
export function messageInfo(sessionID, id, created, role = "user") {
  return role === "assistant" ? assistantInfo(sessionID, id, created) : userInfo(sessionID, id, created);
}
...
export function filePart(sessionID, messageID, id, filePath, sourceText, mime = "application/octet-stream") {
  return {
    id,
    sessionID,
    messageID,
...
export async function captureMessage(store, { sessionID, messageID, created, role = "user", parts = [] }) {
  await store.capture({
