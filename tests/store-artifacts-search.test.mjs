...
test("large repeated content is externalized, deduplicated, and capture cleanup prevents new orphans", async () => {
  ...
  try {
    store = new SqliteLcmStore(workspace, makeOptions({ largeContentThreshold: 40, artifactViewChars: 400 }));
    await store.init();
...
test("search prefers direct message hits, indexes artifact metadata, and falls back to scan", async () => {
  ...
  try {
    store = new SqliteLcmStore(workspace, makeOptions({ freshTailMessages: 1, largeContentThreshold: 50 }));
    await store.init();

    const attachmentPath = writeFixtureFile(workspace, "fixtures/evidence.bin", Buffer.from("evidence-bytes-1234"));
    ...
    const attachmentResults = await store.grep({ query: "evidence.bin", sessionID: "s1", limit: 5 });
    const attachmentResult = attachmentResults.find((result) => result.type.startsWith("artifact:file"));
    ...
    const attachmentArtifact = await store.artifact({ artifactID: attachmentResult.id, chars: 400 });
