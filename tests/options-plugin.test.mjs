...
test("resolveOptions normalizes malformed plugin config", () => {
  ...
  assert.deepEqual(resolved.scopeProfiles, [{ worktree: "C:/repo/a", grep: "all", describe: "session" }]);
...
test("plugin exposes tools, records events, and appends compaction context once", async () => {
  ...
  try {
    const hooks = await OpencodeLcmPlugin(makePluginContext(workspace), makeOptions({ freshTailMessages: 1 }));
    ...
    await hooks.event({
      event: { type: "session.created", properties: { sessionID: "s1", info: sessionInfo(workspace, "s1", 1) } },
    });
